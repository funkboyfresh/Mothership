...
// --- NEW MONTE CARLO COORDINATE GENERATOR ---
function getSafeCoordinates(existingMissions) {
    const activeWire = (existingMissions || []).slice(-8);
    const count = (existingMissions || []).length;
    
    // Node 0: Organic Constellation Start (Avoid center & corners)
    if (count === 0) {
        const angle = Math.random() * Math.PI * 2;
        const radius = 18 + Math.random() * 15; // Spawns in a comfortable mid-ring
        return {
    ...
    // Simulate 3000 drops and score them continuously
    for (let attempts = 0; attempts < 3000; attempts++) {
        let x = 12 + Math.random() * 76; // Coordinates restricted from 12% to 88% screen width
        let y = 12 + Math.random() * 76;
        const candidate = { x, y };

        let score = 0;

        // 1. Spacing Reward: Distance to nearest node
        ...
        // 2. Node Overlap Penalty
        if (minNodeDist < 18) score -= 100000; 

        if (activeWire.length > 0) {
            const lastNode = activeWire[activeWire.length - 1];
            let segmentDist = Math.sqrt((lastNode.x - x)**2 + (lastNode.y - y)**2);
            
            // 3. Segment Length Penalties (Keep lines organic, avoid extremely long jumps)
            if (segmentDist > 40) score -= (segmentDist - 40) * 100;
            if (segmentDist < 20) score -= (20 - segmentDist) * 100;

            // 4. Line Crossing Penalty (Fatal)
            if (activeWire.length > 2) {
                for (let i = 0; i < activeWire.length - 2; i++) {
                    if (doLinesIntersect(lastNode, candidate, activeWire[i], activeWire[i+1])) {
                        score -= 500000; 
                    }
                }
            }

            // 5. Wire/Node Clearance Penalty (Deflect wires away from old nodes)
            for (let m of existingMissions) {
                if (m.id === lastNode.id) continue;
                let d = getDistanceToSegment(m, lastNode, candidate);
                if (d < 12) score -= 50000;
            }

            // 6. Node/Wire Clearance Penalty (Deflect new nodes away from old wires)
            if (activeWire.length > 1) {
                for (let i = 0; i < activeWire.length - 1; i++) {
                    let d = getDistanceToSegment(candidate, activeWire[i], activeWire[i+1]);
                    if (d < 12) score -= 50000;
                }
            }
            
            // 7. Constellation Angle Penalty (Prevent exact 180 U-turns)
            if (activeWire.length > 1) {
                const prevNode = activeWire[activeWire.length - 2];
                const v1 = { x: lastNode.x - prevNode.x, y: lastNode.y - prevNode.y };
                const v2 = { x: x - lastNode.x, y: y - lastNode.y };
                const dotProduct = v1.x * v2.x + v1.y * v2.y;
                const mag1 = Math.sqrt(v1.x**2 + v1.y**2);
                const mag2 = Math.sqrt(v2.x**2 + v2.y**2);
                const cosTheta = dotProduct / (mag1 * mag2);
                if (cosTheta < -0.5) score -= 5000 * Math.abs(cosTheta + 0.5); 
            }
        }

        // Keep candidates safely away from literal UI edges
        if (x < 15 || x > 85 || y < 15 || y > 85) score -= 20000;
    ...
    // Because it is a continuous score, bestCandidate is NEVER null. It always picks the least penalized spot.
...
function renderLevel3(container, footer) {
    ...
    [...debrisMissions, ...wireTasks].forEach((m) => {
        ...
        if (isDebris) {
            ...
            node.style.backgroundColor = archivalColor; // Core fill is warning color
            node.style.borderColor = accentColor; // Outline is sector color
            node.style.borderWidth = '2px';
            node.style.borderStyle = 'solid';
            node.style.boxShadow = `0 0 10px ${archivalColor}99`; // Glow matches fill
