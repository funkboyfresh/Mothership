...
function getSafeCoordinates(existingMissions) {
    ...
    for (let attempts = 0; attempts < 2500; attempts++) {
        ...
        if (score > bestScore) { bestScore = score; bestCandidate = newNode; }
    }
...
function renderLevel1(container, footer) {
    ...
    state.sectors.forEach((s, i) => {
        ...
        path.setAttribute("stroke", color); path.setAttribute("stroke-width", "2"); path.setAttribute("class", `voronoi-cell ${overdue ? 'overdue-sector' : ''}`);
        ...
        clipPathEl.setAttribute("d", pathData); clipPath.appendChild(clipPathEl); defs.appendChild(clipPath);
        const floatGroup = document.createElementNS("http://www.w3.org/2000/svg", "g");
        floatGroup.setAttribute("clip-path", `url(#clip-${s.id})`); svg.appendChild(floatGroup);
        let completedMissions = [];
        HORIZONS.forEach(h => { if (state.missions[s.id] && state.missions[s.id][h]) { completedMissions.push(...state.missions[s.id][h].filter(m => m.captured)); } });
        ...
        completedMissions.forEach((m, idx) => {
            ...
            const px = cx + (Math.random() - 0.5) * (w * 0.7), py = cy + (Math.random() - 0.5) * (h * 0.7);
            particle.setAttribute("cx", px); particle.setAttribute("cy", py); particle.setAttribute("r", "2.25");
            let pColor = s.color; if (m.overdue) pColor = 'var(--thrust)'; else if (m.warningLevel === 24) pColor = '#ff9900'; else if (m.warningLevel === 48) pColor = '#ffd700';
            particle.setAttribute("fill", pColor); particle.setAttribute("opacity", "0.6");
            const animX = document.createElementNS("http://www.w3.org/2000/svg", "animate");
            animX.setAttribute("attributeName", "cx"); animX.setAttribute("values", `${px}; ${px + (Math.random()-0.5)*40}; ${px}`);
            animX.setAttribute("dur", `${15 + Math.random()*20}s`); animX.setAttribute("repeatCount", "indefinite");
            const animY = document.createElementNS("http://www.w3.org/2000/svg", "animate");
            animY.setAttribute("attributeName", "cy"); animY.setAttribute("values", `${py}; ${py + (Math.random()-0.5)*40}; ${py}`);
            animY.setAttribute("dur", `${15 + Math.random()*20}s`); animY.setAttribute("repeatCount", "indefinite");
            particle.appendChild(animX); particle.appendChild(animY); floatGroup.appendChild(particle);
        });
        ...
        rings.forEach(ring => {
            ...
            circle.setAttribute("fill", "none"); circle.setAttribute("stroke", color); circle.setAttribute("stroke-width", "0.5"); circle.setAttribute("opacity", "0.4"); svg.appendChild(circle);
            const missions = (state.missions[s.id]?.[ring.id] || []).filter(m => !m.captured);
            if (missions.length > 0) {
                const group = document.createElementNS("http://www.w3.org/2000/svg", "g");
                group.style.transformOrigin = `${cx}px ${cy}px`; group.style.animation = `orbit-spin ${ring.s}s linear infinite`;
                missions.forEach((m, idx) => {
                    ...
                    let dotColor = color; if (m.overdue) dotColor = 'var(--thrust)'; else if (m.warningLevel === 24) dotColor = '#ff9900'; else if (m.warningLevel === 48) dotColor = '#ffd700';
                    dot.setAttribute("fill", dotColor); group.appendChild(dot);
        ...
        text.style.pointerEvents = "none"; text.style.textShadow = `0 0 10px ${color}`; text.textContent = s.name; svg.appendChild(text);
        const sectorCaptured = getCapturedCount(s.id);
        const subText = document.createElementNS("http://www.w3.org/2000/svg", "text");
        subText.setAttribute("x", cx); subText.setAttribute("y", cy + 62); subText.setAttribute("fill", color); 
        subText.style.pointerEvents = "none"; subText.style.fontSize = "0.5rem"; subText.style.letterSpacing = "2px"; subText.style.opacity = "0.7";
        subText.style.textShadow = `0 0 8px ${color}`; subText.setAttribute("text-anchor", "middle"); subText.textContent = `[ ${sectorCaptured} SECURED ]`; svg.appendChild(subText);
    });
}
...
function renderLevel2(container, footer, activeSector) {
    ...
    sectorTracker.innerHTML = `<div style="opacity: 0.5; font-size: 0.5rem; text-transform: uppercase;">SECTOR SECURED</div><div style="font-size: 1.2rem; font-weight: bold; color: var(--accent); text-shadow: 0 0 10px var(--accent-glow);">${sectorCaptured} <span style="font-size: 0.8rem;">★</span></div>`;
    ...
    for (let i = 0; i < 225; i++) {
        const p = document.createElement('div'), r = 30 + (Math.random() * 145), angle = Math.random() * Math.PI * 2;
        const x = 140 + r * Math.cos(angle), y = 140 + r * Math.sin(angle), size = Math.random() * 1.5 + 0.5;
        p.style.cssText = `position:absolute; width:${size}px; height:${size}px; background:#fff; border-radius:50%; opacity:${Math.random() * 0.4 + 0.1}; left:${x}px; top:${y}px; animation: orbit-spin ${40 + Math.random() * 80}s linear infinite;`; gravityWell.appendChild(p);
    }
    center.appendChild(gravityWell);
    const textSvg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    textSvg.style.cssText = 'position:absolute; width:100%; height:100%; pointer-events:none; z-index:20;'; textSvg.setAttribute("viewBox", "0 0 280 280");
    textSvg.innerHTML = `<defs><path id="path-horizon" d="M 67.5,140 A 72.5,72.5 0 0,1 212.5,140" /><path id="path-trajectory" d="M 22.5,140 A 117.5,117.5 0 0,1 257.5,140" /></defs>`; center.appendChild(textSvg);
    const views = [ { id: 'TRAJECTORY', size: 280, speed: 60 }, { id: 'HORIZON', size: 190, speed: 30 }, { id: 'IMMINENT', size: 100, speed: 15 } ];
    views.forEach(d => {
        ...
        if (d.id === 'IMMINENT') {
            ...
            const text = document.createElementNS("http://www.w3.org/2000/svg", "text"); text.setAttribute("fill", "#ffffff"); 
            text.style.cssText = 'font-size: 0.7rem; letter-spacing: 2px; font-weight: bold; text-transform: uppercase;';
            const tp = document.createElementNS("http://www.w3.org/2000/svg", "textPath"); tp.setAttribute("href", `#path-${d.id.toLowerCase()}`); 
            tp.setAttribute("startOffset", "50%"); tp.setAttribute("text-anchor", "middle"); tp.setAttribute("dominant-baseline", "middle"); tp.textContent = d.id; text.appendChild(tp); textSvg.appendChild(text);
        }
        ...
        missions.forEach((m, i) => {
            const angle = (i / missions.length) * Math.PI * 2, r = d.size/2, dot = document.createElement('div'); 
            let dotColor = activeSector.color; if (m.overdue) dotColor = 'var(--thrust)'; else if (m.warningLevel === 24) dotColor = '#ff9900'; else if (m.warningLevel === 48) dotColor = '#ffd700';
...
function renderLevel3(container, footer) {
    ...
    const missions = state.missions[state.sectorId]?.[state.horizon] || [], svg = document.createElementNS("http://www.w3.org/2000/svg", "svg"); 
    ...
    if (wireActive.length > 0) {
        ...
        pc.innerHTML = `<button class="priority-toggle-btn" onclick="this.nextElementSibling.classList.toggle('show')">MISSION PRIORITIES (${wireActive.length}/6) <span>v</span></button>
            <div class="priority-list">${wireActive.map((m, i) => { const isD = m.overdue && !m.captured;
                return `<div class="priority-item ${i === 0 ? 'mission-critical-active' : ''}" style="${i === 0 ? `--sector-color: ${isD ? 'rgba(255, 42, 42, 0.2)' : accentColor + '22'}; --sector-border: ${isD ? 'var(--thrust)' : accentColor};` : ''}">
    ...
    if (wireTasks.length > 1) {
        for (let i = 0; i < wireTasks.length - 1; i++) {
            ...
            line.setAttribute("stroke-dasharray", "5,5"); line.setAttribute("opacity", "0.45"); svg.appendChild(line);
        }
    }
    [...debrisMissions, ...wireTasks].forEach((m) => {
        const star = document.createElement('div'), isDebris = debrisMissions.includes(m), isCw = wireCaptured.includes(m), isDecay = m.overdue;
        let wc = ''; if (isDecay && !m.captured) wc = 'decaying'; else if (m.warningLevel === 24) wc = 'warning-24'; else if (m.warningLevel === 48) wc = 'warning-48';
        star.className = `star-container ${isDebris ? 'debris-node' : ''} ${wc} warp-transition`;
        ...
        if (isDebris) { 
            let wf = accentColor; if (isDecay) wf = 'var(--thrust)'; else if (m.warningLevel === 24) wf = '#ff9900'; else if (m.warningLevel === 48) wf = '#ffd700';
            node.style.transform = `scale(${m.scale})`; node.style.opacity = '0.35'; node.style.backgroundColor = wf;
            node.style.borderColor = accentColor; node.style.borderWidth = '1px'; node.style.borderStyle = 'solid'; node.style.boxShadow = 'none'; 
        } else if (isCw) {
            let wf = accentColor; if (isDecay) wf = 'var(--thrust)'; else if (m.warningLevel === 24) wf = '#ff9900'; else if (m.warningLevel === 48) wf = '#ffd700';
            node.style.opacity = '1.0'; node.style.backgroundColor = wf; node.style.borderColor = accentColor; node.style.borderWidth = '2px';
            node.style.borderStyle = 'solid'; node.style.color = '#ffffff'; node.style.textShadow = '0px 0px 3px rgba(0,0,0,0.8)';
            node.style.boxShadow = `0 0 10px ${wf}99`; node.textContent = missions.indexOf(m) + 1;
        } else {
            const isC = m.id === wireActive[0]?.id, op = isC ? 1.0 : 0.8, hex = Math.floor(op * 255).toString(16).padStart(2, '0');
            let bc = accentColor, gc = accentColor + hex;
            if (isDecay) { bc = 'var(--thrust)'; gc = 'rgba(255, 42, 42, 0.6)'; } else if (m.warningLevel === 24) { bc = '#ff9900'; gc = 'rgba(255, 153, 0, 0.6)'; } else if (m.warningLevel === 48) { bc = '#ffd700'; gc = 'rgba(255, 215, 0, 0.6)'; }
            node.style.boxShadow = `0 0 ${isC ? 20 : 15}px ${gc}`; node.style.borderColor = bc; node.style.filter = `brightness(${isC ? 1.15 : 1.0})`; if (isC) node.style.borderWidth = '3px'; node.textContent = missions.indexOf(m) + 1;
        }
        const label = document.createElement('div'); label.className = 'star-label'; label.style.display = isDebris ? 'none' : 'block'; label.style.opacity = isCw ? '0.45' : '1'; label.textContent = m.name; star.appendChild(node); star.appendChild(label); container.appendChild(star);
    });
}
...
function renderLevel4(container, footer) {
    ...
    const lock = document.createElement('div'), isD = m.overdue && !m.captured; if (isD) document.getElementById('app').classList.add('critical-mode');
    lock.className = `target-lock warp-transition ${isD ? 'critical' : ''}`; lock.innerHTML = `<div class="view-level-title">LEVEL 4 // ${m.captured ? 'ARCHIVE' : 'ACTIVE'}</div><h2 style="color: ${isD ? 'var(--thrust)' : 'var(--text)'}">${m.name}</h2>`;
    ...
    if (m.subs.length > 0) {
        ...
        pc.innerHTML = `<button class="priority-toggle-btn" onclick="this.nextElementSibling.classList.toggle('show')">MISSION PRIORITIES <span>v</span></button>
            <div class="priority-list">${m.subs.map((s, i) => `<div class="priority-item ${i === ci && !s.c ? 'mission-critical-active' : ''} ${s.c ? 'task-captured' : ''}" style="${i === ci && !s.c ? `--sector-color: ${acc}22; --sector-border: ${acc};` : ''}">
                <span class="p-num">${i + 1}</span><span class="p-text">${s.t}</span></div>`).join('')}</div>`; lock.appendChild(pc);
    }
    ...
    const os = document.createElement('div'); os.className = 'orbital-system'; os.innerHTML = `<svg viewBox="0 0 340 340" style="position:absolute; width:100%; height:100%;"><circle cx="170" cy="170" r="14" fill="${allDone ? 'var(--captured)' : 'var(--bg)'}" stroke="var(--accent)" stroke-width="2"/></svg>`;
    m.subs.forEach((s, i) => {
        const angle = (i / m.subs.length) * Math.PI * 2, x = 170 + 100 * Math.cos(angle), y = 170 + 100 * Math.sin(angle), sn = document.createElement('div'); 
        sn.style.cssText = `position:absolute; left:${x}px; top:${y}px; transform:translate(-50%, -50%); cursor:pointer;`; if (!m.captured) sn.onclick = () => toggleSubTask(i);
        const box = document.createElement('div'); box.className = `orbital-node-box ${s.c ? 'checked' : ''}`; box.innerHTML = `<div class="orb-check">${s.c ? '✓' : ''}</div><div class="orb-text">${s.t}</div>`; sn.appendChild(box); os.appendChild(sn);
    });
    lock.appendChild(os);
    ...
    lock.appendChild(bw); if (allDone && !m.captured) {
        const mdl = document.createElement('div'); mdl.className = 'hex-modal warp-transition'; mdl.innerHTML = `<h2 style="color: var(--captured)">TARGET SECURED</h2><button class="success-btn" onclick="completeMission()">LOG MISSION & WARP</button>`; lock.appendChild(mdl);
    }
...
function openTaskModal(h, f) { 
    ...
    const timeIn = document.getElementById('modal-task-time'); if(timeIn) timeIn.value = '';
...
function renderModalSubtasks() {
    ...
    tempSubtasks.forEach((sub, i) => {
        ...
        row.innerHTML = `<input type="text" class="modal-input" value="${sub}" placeholder="optional - enter sub routine" oninput="tempSubtasks[${i}] = this.value" style="background:transparent; border-color:rgba(255,255,255,0.1);"><button class="subtask-remove-minimal" onclick="tempSubtasks.splice(${i}, 1); renderModalSubtasks();">−</button>`;
...
function saveTaskModal() {
    ...
    if (!editModeId && ac >= 6) { alert("Current mission must be completed first."); return; }
    ...
    if (editModeId) {
        ...
        if (ei !== -1) {
            if (eh === finalH) { const m = state.missions[state.sectorId][finalH][ei]; m.name = name; m.dueDate = dateStr || null; m.dueTime = timeStr; m.subs = tempSubtasks.filter(t => t.trim()).map(t => ({t, c:false})); }
            else { state.missions[state.sectorId][eh].splice(ei, 1); const crds = getSafeCoordinates(state.missions[state.sectorId][finalH] || []); state.missions[state.sectorId][finalH].push({ id: editModeId, name, subs: tempSubtasks.filter(t => t.trim()).map(t => ({t, c:false})), x: crds.x, y: crds.y, dueDate: dateStr || null, dueTime: timeStr }); }
        }
    } else { const crds = getSafeCoordinates(hms); state.missions[state.sectorId][finalH].push({ id: Date.now(), name, subs: tempSubtasks.filter(t => t.trim()).map(t => ({t, c:false})), x: crds.x, y: crds.y, dueDate: dateStr || null, dueTime: timeStr }); }
...
function resetSectorMissions(sid) { if(confirm("Wipe missions?")) { state.missions[sid] = { TRAJECTORY: [], HORIZON: [], IMMINENT: [] }; save(); render(); } }
function factoryReset() { if(confirm("TERMINAL ACTION: Wipe all data?")) { localStorage.clear(); location.reload(); } }
...
function openEditModal(id) {
    ...
    const timeIn = document.getElementById('modal-task-time'); if(timeIn) timeIn.value = target.dueTime || '';
...
function togglePilotLog() {
    let logModal = document.getElementById('pilot-log-modal'); if (!logModal) { logModal = document.createElement('div'); logModal.id = 'pilot-log-modal'; logModal.className = 'modal-overlay'; logModal.onclick = (e) => { if(e.target === logModal) logModal.style.display = 'none'; }; document.body.appendChild(logModal); }
...
function runDatabaseMigration() {
    ...
    state.missions = nM; state.sectors.forEach(s => { if (!state.missions[s.id]) { state.missions[s.id] = {TRAJECTORY:[], HORIZON:[], IMMINENT:[]}; m = true; } 
        HORIZONS.forEach(h => { state.missions[s.id][h].forEach((mi, idx, arr) => { if (mi.x === undefined) { let c = getSafeCoordinates(arr.slice(0, idx)); mi.x = c.x; mi.y = c.y; m = true; } }); });
    }); if (m) save();
}
