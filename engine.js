...
function renderLevel3(container, footer) {
    ...
    if (activeTarget) {
        ...
        ship.style.cssText = `transform-origin: 0 0; animation: ship-patrol 4s linear infinite;`;
...
function handleDragStart(e, index) { dragStartIndex = index; e.target.closest('.subtask-row').classList.add('dragging'); }
...
function handleDrop(e, targetIndex) {
    ...
    dragStartIndex = null; renderModalSubtasks();
}

function renderModalSubtasks() {
    const list = document.getElementById('modal-subtasks-list'); if(!list) return; list.innerHTML = '';
    tempSubtasks.forEach((sub, i) => {
        const row = document.createElement('div'); row.className = 'subtask-row'; row.draggable = true;
        row.ondragstart = (e) => handleDragStart(e, i); row.ondragover = (e) => handleDragOver(e); row.ondrop = (e) => handleDrop(e, i);
        row.ondragend = (e) => e.target.classList.remove('dragging');
        row.innerHTML = `<div class="drag-handle">></div><input type="text" class="modal-input" value="${sub}" oninput="tempSubtasks[${i}] = this.value" style="background:transparent; border-color:rgba(255,255,255,0.1);"><button class="subtask-remove-minimal" onclick="tempSubtasks.splice(${i}, 1); renderModalSubtasks();">−</button>`;
...
function moveMission(direction) {
    ...
    if (newIndex >= 0 && newIndex < missions.length) { missions.splice(index, 1); missions.splice(newIndex, 0, m); save(); render(); }
}
